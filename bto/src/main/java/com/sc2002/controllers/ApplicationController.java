...
/**
 * Handles requests related to BTO project applications, acting as an
 * intermediary between the user interface (views) and the application logic
 * (ApplicationService). Future implementation of input validation can be
 * handled here.
 */
public class ApplicationController {
    ...
    /**
     * Default constructor for ApplicationController. Note: The
     * ApplicationService is not initialized here. Use the constructor with
     * AppContext or set the service manually if using this constructor.
     */
     ...
     *
     * @param appContext The shared application context containing currentUser,
    ...
    /**
     * Constructs an ApplicationController with a pre-existing
     * ApplicationService instance. Useful for testing or specific dependency
     * injection scenarios.
     *
    ...
    /**
     * Uses getAvailableProjectsForApplicant() to print, just a depriciated
     * method. Delegates the calling getAvailableProjectsForApplicant() logic to
    ...
    /**
     * Retrieves a list of BTO projects that the current applicant is eligible
     * to apply for. Delegates the logic to the ApplicationService.
     *
     * @return A list of BTOProjectModel instances representing the available
    ...
    /**
     * Handles the process for a user to submit an application for a specific
     * BTO project. Delegates the application logic to the ApplicationService.
     *
     * @param projectRepo Repository to access project data.
     * @param scanner Scanner instance for capturing user input (e.g., flat type
     * choice).
     * @param currentUser The user model representing the applicant.
     * @return True if the application submission was successful, false
     * otherwise.
     */
     ...
     * @return The ApplicationStatus enum value representing the application's
     * current state.
     */
    ...
    /**
     * Sets the status of a given BTO application to SUCCESSFUL. Note: This is a
     * simple status update. Use {@code approveApplicantApplication} for logic
    ...
    /**
     * Sets the status of a given BTO application to UNSUCCESSFUL. Note: This is
     * a simple status update. Use {@code rejectApplicantApplication} for logic
     * involving checks and balances.
     *
     ...
     *
     * @param application The BTOApplicationModel instance for which to generate
     * a receipt.
    ...
    /**
     * Handles the logic for approving an applicant's BTO application, including
     * necessary checks (e.g., flat availability). Delegates the core approval
     * logic to the ApplicationService.
     *
     * @param application The BTOApplicationModel instance to approve.
     * @return True if the application was successfully approved, false
     * otherwise (e.g., no flats left, authorization failure).
     */
    ...
    /**
     * Handles the logic for rejecting an applicant's BTO application. Delegates
     * the core rejection logic to the ApplicationService.
     *
     * @param application The BTOApplicationModel instance to reject.
     * @return True if the application was successfully rejected, false
     * otherwise (e.g., authorization failure).
     */
    ...
    /**
     * Handles the logic for approving an applicant's request to withdraw their
     * BTO application. This typically involves updating status and potentially
     * adjusting flat counts. Delegates the core withdrawal approval logic to
     * the ApplicationService.
     *
     * @param application The BTOApplicationModel instance whose withdrawal
    ...
    /**
     * Handles the logic for rejecting an applicant's request to withdraw their
     * BTO application. This typically involves resetting the withdrawal request
     * flag. Delegates the core withdrawal rejection logic to the
     * ApplicationService.
     *
     * @param application The BTOApplicationModel instance whose withdrawal
    ...
    /**
     * Updates the status of a specific application to BOOKED. Delegates the
     * update logic to the ApplicationService.
     *
    ...
    /**
     * Updates the chosen flat type for a specific application, ensuring the new
     * type is valid. Delegates the update logic to the ApplicationService.
     *
     * @param applicationID The unique ID of the application to update.
     * @param newFlatType The desired new FlatType.
     * @param availableFlatTypes A list of currently available flat types for
     * validation.
     * @return True if the flat type was successfully updated, false otherwise
     * (e.g., invalid type, application not found).
     */
    ...
    /**
     * Searches for and retrieves a specific BTO application within a project
     * based on the applicant's NRIC. Delegates the search logic to the
     * ApplicationService.
     *
     * @param projectID The ID of the project to search within.
     * @param nric The NRIC of the applicant whose application is sought.
     * @param outputApplication An array (expected size 1) to store the found
     * application if successful.
     * @return True if an application matching the criteria was found and placed
     * in {@code outputApplication}, false otherwise.
     */
